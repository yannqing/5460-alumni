  ...
  async loadActivityDetail(activityId) {
    try {
      ...
      if (res.data && (res.data.code === 0 || res.data.code === 200) && res.data.data) {
        ...
        const registrationStartTime = activityData.registrationStartTime
          ? new Date(activityData.registrationStartTime)
          : new Date()
        const registrationEndTime = activityData.registrationEndTime
          ? new Date(activityData.registrationEndTime)
          : new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)
