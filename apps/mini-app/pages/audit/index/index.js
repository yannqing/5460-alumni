  ...
  async checkPermissions() {
    ...
    if (hasLocalAdmin || localFromApi) {
      filteredSchoolOfficeFunctions = this.data._allSchoolOfficeFunctions.filter(item => {
        ...
        if (item.name === '成员管理') return this.hasPermission('LOCAL_PLATFORM_MEMBER_MANAGEMENT') || hasLocalAdmin
        if (item.name === '架构管理') return this.hasPermission('LOCAL_PLATFORM_ARCHIVE_MANAGEMENT') || hasLocalAdmin
        if (item.name === '资讯管理') return this.hasPermission('LOCAL_PLATFORM_ARTICLE_MANAGEMENT') || hasLocalAdmin
        if (item.name === '校友会认证') return this.hasPermission('SYSTEM_ALUMNI_ASSOCIATION_CERTIFICATION') || cityModule || hasLocalAdmin
        if (item.name === '信息维护') return this.hasPermission('SYSTEM_ALUMNI_ASSOCIATION_MAINTENANCE') || cityModule || hasLocalAdmin
        return false
      })
    }
    ...
    if (hasAlumniAdmin || alumniFromApi) {
      filteredAlumniFunctions = this.data._allAlumniFunctions.filter(item => {
        ...
        if (item.name === '架构管理') return this.hasPermission('ALUMNI_ASSOCIATION_ARCHIVE_MANAGEMENT') || hasAlumniAdmin
        if (item.name === '成员管理') return this.hasPermission('ALUMNI_ASSOCIATION_MEMBER_MANAGEMENT') || hasAlumniAdmin
        if (item.name === '商户审核') return this.hasPermission('ALUMNI_ASSOCIATION_MERCHANT_MANAGEMENT') || this.hasPermission('ALUMNI_ASSOCIATION_MERCHANT_AUDIT') || hasAlumniAdmin
        if (item.name === '店铺审核') return this.hasPermission('ALUMNI_ASSOCIATION_SHOP_REVIEW') || hasAlumniAdmin
        if (item.name === '加入审核') return this.hasPermission('ALUMNI_ASSOCIATION_JOIN_REVIEW') || hasAlumniAdmin
        if (item.name === '活动管理') return this.hasPermission('ALUMNI_ASSOCIATION_ACTIVITY_MANAGEMENT') || hasAlumniAdmin
        if (item.name === '企业管理') return this.hasPermission('ALUMNI_ASSOCIATION_ENTERPRISE_MANAGEMENT') || hasAlumniAdmin
        if (item.name === '信息维护') return this.hasPermission('ALUMNI_ASSOCIATION_INFORMATION') || alumniModule || hasAlumniAdmin
        if (item.name === '资讯管理') return this.hasPermission('ALUMNI_ASSOCIATION_ARTICLE_MANAGEMENT') || hasAlumniAdmin
        return false
      })
    }
    ...
    if (hasMerchantAdmin || hasShopAdmin || merchantFromApi) {
      filteredMerchantFunctions = this.data._allMerchantFunctions.filter(item => {
        // 如果是商户管理员，按具体权限码过滤
        if (hasMerchantAdmin || merchantFromApi) {
          if (item.name === '店铺管理') return this.hasPermission('MERCHANT_SHOP_MANAGEMENT') || hasMerchantAdmin
          if (item.name === '成员管理') return this.hasPermission('MERCHANT_MEMBER_MANAGEMENT') || hasMerchantAdmin
          if (item.name === '优惠券管理') return this.hasPermission('MERCHANT_COUPON_MANAGEMENT') || hasMerchantAdmin
          if (item.name === '核销优惠券') return this.hasPermission('MERCHANT_DEAL_COUPON') || hasMerchantAdmin || hasShopAdmin
          if (item.name === '话题管理') return this.hasPermission('MERCHANT_TOPIC_MANAGEMENT') || hasMerchantAdmin
          if (item.name === '信息维护') return this.hasPermission('MERCHANT_ASSOCIATION_INFORMATION') || hasMerchantAdmin
        } 
