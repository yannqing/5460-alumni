      ...
      .then(res => {
        if (res.data.code === 200) {
          ...
          const endTime = activityInfo.endTime
            ? new Date(activityInfo.endTime)
            : new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)
          const registrationStartTime = activityInfo.registrationStartTime
            ? new Date(activityInfo.registrationStartTime)
            : new Date()
          const registrationEndTime = activityInfo.registrationEndTime
            ? new Date(activityInfo.registrationEndTime)
            : new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)
          ...
          this.setData({
            ...
            formData: {
              ...
              registrationStartTime:
                activityInfo.registrationStartTime ||
                this.formatDateToPicker(registrationStartTime),
              registrationEndTime:
                activityInfo.registrationEndTime || this.formatDateToPicker(registrationEndTime),
            ...
            activityImageList: activityInfo.activityImages
              ? JSON.parse(activityInfo.activityImages).map(url => ({ url }))
              : [],
        ...
        setTimeout(() => {
          if (type === 'cover') {
            ...
            const activityImageList = [
              ...that.data.activityImageList,
              ...tempFilePaths.map(path => ({ url: path })),
            ]
            that.setData({
