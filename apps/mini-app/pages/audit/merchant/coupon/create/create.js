...
Page({
  ...
  validate() {
    ...
    const validStartTime = toDateTimeIso(formData.validStartDate, formData.validStartTime)
    const validEndTime = toDateTimeIso(formData.validEndDate, formData.validEndTime)
    ...
    if (payload.publishType === 2) {
      const publishTime = toDateTimeIso(formData.publishDate, formData.publishTime)
      if (!publishTime) {
