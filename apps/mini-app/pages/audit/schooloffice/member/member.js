...
const { alumniApi, localPlatformManagementApi, localPlatformApi, userApi } = require('../../../../api/api.js')
const app = getApp()
...
Page({
  ...
  async loadSchoolOfficeList() {
    try {
      ...
      const schoolOfficeList = organizationList.map(org => {
        let logo = org.logo || ''
        ...
        return {
          id: org.id,
          schoolOfficeId: org.id,
          schoolOfficeName: org.name || '校促会',
          logo: logo,
      ...
      this.setData({ schoolOfficeList })
      console.log('[Debug] 校促会列表:', schoolOfficeList)
      this.handleSchoolOfficeSelection(schoolOfficeList)
    } catch (error) {
